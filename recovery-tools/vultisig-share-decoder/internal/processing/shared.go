	...
	if ctx.EdDSAPublicKeyHex != "" && ctx.EdDSAPrivateKeyHex != "" {
		...
		if err != nil {
			...
			if err != nil {
				...
				// CRITICAL FIX: For DKLS, call EdDSA handlers directly with the correct key bytes
				// Do NOT use EdDSAKeyProcessor as it corrupts already-correct DKLS keys
				eddsaCoins := GetEnhancedEdDSACoins()
				for _, coin := range eddsaCoins {
					log.Printf("Processing DKLS EdDSA coin: %s", coin.Name)
					Debug().Emit(DEBUG, "DKLS-EdDSA", "Processing DKLS EdDSA coin", map[string]any{
						"coin":        coin.Name,
