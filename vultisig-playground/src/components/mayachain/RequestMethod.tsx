...
export function RequestMethod({ provider, onResult, onError, onAccountUpdate }: RequestMethodProps) {
  ...
  const handleExecute = async (): Promise<void> => {
    const mayaProvider = provider as {
      request?: (args: { method: string; params?: unknown[] }) => Promise<unknown>
    } | null

    if (!mayaProvider?.request || typeof mayaProvider.request !== 'function') {
      onError('MayaChain provider or request method not available')
      return
    }
    ...
    setLoading(true)
    try {
      const result = await mayaProvider.request(requestPayload)
      onResult(result)
        ...
        <>
          ...
          <div>
            ...
            <input
              ...
              placeholder="0 (1 CACAO = 1e10 base units)"
              className="w-full px-3 py-2 text-xs font-mono border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
